...
namespace dataset
{
    ...
    private:
        ...
        std::unordered_map<std::string, neurons::Matrix> m_word_vec_dic;
        ...
        std::vector<neurons::Matrix> m_inputs;
        std::vector<neurons::Matrix> m_labels;

        size_t m_arbitrary_review_len;

    private:

        void read_word_vec_dic(std::unordered_map<std::string, neurons::Matrix> &dic, const std::string & word_vec_file);
        ...
        neurons::Matrix review_to_matrix(const std::vector<std::string> & review) const;
